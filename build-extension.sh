...
rm -rf build dist extension/granola-organizer-mcp
"$VENV/bin/pyinstaller" \
  --onefile --console --clean --noconfirm \
  --name granola-organizer-mcp \
  ...
  --hidden-import granola_organizer.service \
  --hidden-import granola_organizer.mcp_server \
  --hidden-import granola_organizer.cli \
  entry_mcp.py

cp dist/granola-organizer-mcp extension/granola-organizer-mcp
chmod +x extension/granola-organizer-mcp
echo "binary: $(du -h extension/granola-organizer-mcp | cut -f1)"
...
./extension/granola-organizer-mcp poll --help > /dev/null || {
...
SMOKE_BIN="$PWD/extension/granola-organizer-mcp"
GRANOLA_ORGANIZER_HOME="$SMOKE_HOME" "$SMOKE_BIN" poll --interval 120 \
...
GRANOLA_ORGANIZER_HOME="$SMOKE_HOME" ./extension/granola-organizer-mcp < /dev/null \
...
if command -v mcpb >/dev/null 2>&1; then
    mcpb validate extension/manifest.json && mcpb pack extension granola-organizer.mcpb
else
    ( cd extension && zip -qr ../granola-organizer.mcpb . )
    echo "packed with zip (install 'mcpb' for manifest validation)"
fi
echo "built: $(du -h granola-organizer.mcpb | cut -f1) granola-organizer.mcpb"
