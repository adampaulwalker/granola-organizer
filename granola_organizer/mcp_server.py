...
logger = logging.getLogger("granola-organizer")

mcp = _Server("granola-organizer", version="1.0.0")
...
def granola_enable_always_on(interval_seconds: int = 120) -> Dict[str, Any]:
    ...
    return {
        ...
        "message": ("Automatic filing is on and confirmed running. It starts by "
                    ...
                    "first, or run 'granola-organizer uninstall' from Terminal."),
    }
...
def main() -> None:
    logger.info("granola-organizer MCP server starting (config: %s)", DATA_DIR)
    mcp.run()
