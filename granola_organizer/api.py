...
def _data_dir() -> Path:
    ...
    `GRANOLA_ORGANIZER_HOME` overrides everything, which is how you run a second
    ...
    Otherwise ~/.granola-organizer, falling back to whichever earlier directory
    exists. This tool has been renamed twice, and each rename would silently
    strand a working install's API key, routing rules and state - the archive
    would look empty and every meeting would be re-filed. Adopting the existing
    directory in place is cheaper and safer than migrating it.
    """
    for var in ("GRANOLA_ORGANIZER_HOME", "GRANOLA_ROUTER_HOME"):
        env = os.environ.get(var)
        if env and env.strip():
            return Path(env).expanduser().resolve()

    current = Path.home() / ".granola-organizer"
    if current.exists():
        return current
    for legacy in (Path.home() / ".granola-router", Path.home() / ".granola-saver"):
        if legacy.exists():
            return legacy
    return current
